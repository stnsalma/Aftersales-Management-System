...
{
    public partial class AreaReport : System.Web.UI.UserControl
    {
        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            ...
            if (UserID != null && SearchBy != null)
            {
                ...
                //string queryString = "select top 20 im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMSTEST.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where im.RequestStatus in ('Approved','Approved Declined','WareHouse Received','WareHouse Declined') and  pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc ";
               // string queryString = "select im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMSTEST.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc ";
                string queryString = "select  im.RequestID,im.IMEI_1,im.IMEI_2,im.Model,im.DealerCode,im.RegistrationDate,im.DistributionDate,im.RequestType,im.Issues,im.RequestStatus,rl.AppDeclinedRemarks from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode left join RBSYNERGY.IMEIReplacementMaster.ReplacementLog rl on rl.RequestID=im.RequestID where pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc ";


                if (search[0] == "Date")
                {
                  
                    //queryString = "select im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMSTEST.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where im.RequestStatus in ('Approved','Approved Declined','WareHouse Received','WareHouse Declined') and cast(CONVERT(varchar(8), im.RequestDate, 112) AS datetime) between '" + search[1] + @" 00:00:00' And '" + search[1] + @" 23:59:59' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc ";
                  //  queryString = "select im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMSTEST.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where cast(CONVERT(varchar(8), im.RequestDate, 112) AS datetime) between '" + search[1] + @" 00:00:00' And '" + search[1] + @" 23:59:59' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc ";
                    queryString = "select  im.RequestID,im.IMEI_1,im.IMEI_2,im.Model,im.DealerCode,im.RegistrationDate,im.DistributionDate,im.RequestType,im.Issues,im.RequestStatus,rl.AppDeclinedRemarks from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode left join RBSYNERGY.IMEIReplacementMaster.ReplacementLog rl on rl.RequestID=im.RequestID where cast(CONVERT(varchar(8), im.RequestDate, 112) AS datetime) between '" + search[1] + @" 00:00:00' And '" + search[1] + @" 23:59:59' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc";
                ...
                {
                   //queryString = "select im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where im.RequestStatus in ('Approved','Approved Declined','WareHouse Received','WareHouse Declined') and im.DealerCode='" + search[1] + "' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc";
                   //queryString = "select im.*,pam.* from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode where im.DealerCode='" + search[1] + "' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc";
                    queryString = "select  im.RequestID,im.IMEI_1,im.IMEI_2,im.Model,im.DealerCode,im.RegistrationDate,im.DistributionDate,im.RequestType,im.Issues,im.RequestStatus,rl.AppDeclinedRemarks from  RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.PlazaAndRSM pam on im.DealerCode=pam.PlazaAndRSMCode left join RBSYNERGY.IMEIReplacementMaster.ReplacementLog rl on rl.RequestID=im.RequestID where im.DealerCode='" + search[1] + "' and pam.ZoneID='" + user.ZoneID + "' order by im.UpdatedDate desc";
