...
{
    public partial class CorporateStoreReport : System.Web.UI.UserControl
    {
        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            ...
            if (UserID != null && SearchBy != null)
            {
                ...
                string queryString = "select wml.AddedDate as WastageReceiveDate,wcl.WastageMgtDeliveryDate,wcl.ReceiveDate as CorporateStoreReceive,wcl.RequestID,im.IMEI_1,im.IMEI_2,im.Model,im.RequestType,im.RequestDate,im.RequestStatus,im.Issues from RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join rbsynergy.IMEIReplacementMaster.WastageManagementMasterLog wml on im.RequestID=wml.RequestID left join RBSYNERGY.IMEIReplacementMaster.WareHouseCorporateStoreLog wcl on im.RequestID=wcl.RequestID where  im.RequestStatus='CorporateStoreDelivered' ";
              
                if (search[0] == "Date1")
                {
                    queryString += "and cast(CONVERT(varchar(8), wcl.ReceiveDate, 112) AS datetime) between '" + search[1] + @" 00:00:00' And '" + search[1] + @" 23:59:59' ";
