        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            if (UserID != null && SearchBy != null)
            {
                ...
                else if (search[0] == "localMade")
                {
                    ...
                    queryString = String.Format(@"select  pam.*,ime.BarCode,ime.BarCode2,ime.DealerCode,ime.Model,ime.DealerdistributionId,ime.DistributionDate, pr.ProductRegID,pr.ProductID, pr.RegistrationDate,
	                    DATEDIFF(day,CONVERT(date,getdate()),DATEADD(day, 30, RegistrationDate)) RequestDateValidity, 
	                    case when  DATEDIFF(day,CONVERT(date,getdate()),DATEADD(day, 30, RegistrationDate))<= 30 then 'YES' end 
	                    Warranty from RBSYNERGY.dbo.tblProductRegistration pr 
	                    ...
	                    WHERE (irpm.IMEI_1 =pr.ProductID or irpm.IMEI_2 =pr.ProductID) ) and (ime.BarCode ='" + search[1] + "' or ime.BarCode2='" + search[1] + "') and tbi.DelieveredTo='SKD' and tbi.Model like '%Primo%' and DATEDIFF(day,CONVERT(date,getdate()),DATEADD(day, 30, RegistrationDate))>=0 and ime.Model like '%Primo%' and  DATEDIFF(day,ime.DistributionDate,GETDATE())<=365");
                }
                else if (search[0] == "localMadeFeature")
                {
                    ...
                    queryString = String.Format(@"select  pam.*,ime.BarCode,ime.BarCode2,ime.DealerCode,ime.Model,ime.DealerdistributionId,ime.DistributionDate, pr.ProductRegID,pr.ProductID, pr.RegistrationDate,
	                    ...
	                    join WSMS.ReplaceMentdb.PlazaAndRSM pam on ime.DealerCode = pam.PlazaAndRSMCode
	                    WHERE NOT EXISTS (SELECT * FROM RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster irpm  
	                    WHERE (irpm.IMEI_1 =pr.ProductID or irpm.IMEI_2 =pr.ProductID) ) and (ime.BarCode ='" + search[1] + "' or ime.BarCode2='" + search[1] + "') and tbi.DelieveredTo='SKD' and tbi.Model like '%Olvio%' and DATEDIFF(day,CONVERT(date,getdate()),DATEADD(day, 30, RegistrationDate))>=0 and ime.Model like '%Primo%' and  DATEDIFF(day,ime.DistributionDate,GETDATE())<=365");
                }
