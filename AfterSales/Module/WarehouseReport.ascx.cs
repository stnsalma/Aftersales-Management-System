...
{
    public partial class WarehouseReport : System.Web.UI.UserControl
    {
        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            ...
            if (UserID != null && SearchBy != null)
            {
                ...
                string queryString = "select im.RequestID,im.RequestDate,wi.ActionDate as WarehouseActionDate,im.IMEI_1,im.IMEI_2,im.Model,par.Organaization_Name,im.RequestStatus, ru.RepUserFullName as ServicePointName, par.PlazaAndRSMCode as ServicePointID,im.Issues from RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join RBSYNERGY.IMEIReplacementMaster.WareHouseApproval wi on wi.RequestID=im.RequestID left join WSMS.ReplaceMentdb.RepUsers ru on im.AddedBy=ru.RepUserID  left join WSMS.ReplaceMentdb.PlazaAndRSM par on par.PlazaAndRSMID=ru.PlazaAndRSMID where  im.RequestStatus in ('WareHouse Received','WareHouse Declined') and wi.AddedBy = '" + UserID + "' "; 
