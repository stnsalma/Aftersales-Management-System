...
{
    public partial class WastageHandsetReceive : System.Web.UI.UserControl
    {
        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            ...
            if (UserID != null && SearchBy != null)
            {
                ...
                string queryString = "select im.AddedBy,ru.RepUserID,ru.ZoneID,im.RequestID,im.RequestDate,im.IMEI_1,im.IMEI_2,im.Model,ru.RepUserFullName as ServicePointName, par.PlazaAndRSMCode as ServicePointID,im.RequestStatus,im.Issues from RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join WSMS.ReplaceMentdb.RepUsers ru on im.AddedBy=ru.RepUserID left join WSMS.ReplaceMentdb.PlazaAndRSM par on par.PlazaAndRSMID=ru.PlazaAndRSMID where im.RequestStatus='ServiceApproved' ";
