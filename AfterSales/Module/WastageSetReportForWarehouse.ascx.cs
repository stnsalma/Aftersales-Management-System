...
{
    public partial class WastageSetReportForWarehouse : System.Web.UI.UserControl
    {
        ...
        public static DataSet GetServiceByUserID(string UserID, string SearchBy)
        {
            ...
            if (UserID != null && SearchBy != null)
            {
                ...
                string queryString = "select im.AddedBy,ru.RepUserID,ru.ZoneID,im.RequestID,wi.AddedDate as WastageReceivedDate, im.RequestDate,wl.DeliveryDate as MovementDateToWarehouse,cl.ReceiveDate as WarehouseReceiveDate,im.IMEI_1,im.IMEI_2, im.Model,ru.RepUserFullName as ServicePointName, par.PlazaAndRSMCode as ServicePointID,im.RequestStatus,im.Issues from RBSYNERGY.IMEIReplacementMaster.IMEIReplacementMaster im left join RBSYNERGY.IMEIReplacementMaster.WastageManagementInventory wi on wi.RequestID=im.RequestID left join RBSYNERGY.IMEIReplacementMaster.WastageManagementMasterLog wl on wl.RequestID=im.RequestID left join RBSYNERGY.IMEIReplacementMaster.WareHouseCorporateStoreLog cl on cl.RequestID=im.RequestID left join WSMS.ReplaceMentdb.RepUsers ru on im.AddedBy=ru.RepUserID left join WSMS.ReplaceMentdb.PlazaAndRSM par on par.PlazaAndRSMID=ru.PlazaAndRSMID where im.RequestStatus='WareHouseDelivered' ";
                if (search[0] == "Date2")
                {
                    queryString += "and cast(CONVERT(varchar(8), cl.ReceiveDate, 112) AS datetime) between '" + search[1] + @" 00:00:00' And '" + search[1] + @" 23:59:59' ";
                }
