...
namespace WSMS
{
    public partial class ReceiveMemo : System.Web.UI.Page
    {
        ...
        protected void btnExportToPDF_Click(object sender, EventArgs e)
        {
            ...
            if (dsn.Tables[0].Rows.Count > 0)
            {
                ...
                //pdfReport.Load(Server.MapPath(@"ServiceReceiveReport.rpt"));

                if (Boolean.Parse(dsn.Tables[0].Rows[0]["WarrantyAvailable"].ToString()))
                {
                    pdfReport.Load(Server.MapPath("~/Reports/ServiceReceiveMemoReport.rpt"));
                }
                else
                {
                    pdfReport.Load(Server.MapPath("~/Reports/ServiceReceiveMemoReportBilling.rpt"));
                }
                ...
                pdfReport.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, true, "ReceiveMemo");
                Response.End();
